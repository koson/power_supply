...
.thumb
	...
	.weak Reset_Handler                                   @ week - значит можно переопределить функцию в коде
	.type Reset_Handler, %function
Reset_Handler:
	ldr   r0, =_estack      /*из stm32f103.ld - указатель на начало стэка*/
	mov   sp, r0          	/* set stack pointer */
	cpsid i					@ disable irq
@	ldr   r0, =SystemInit   @ переход на ф-ю
@	blx   r0
	b    main              @ переход на ф-ю

    .section .text.Default_Handler,"ax",%progbits   @ обработчик дефолтного прерывания (не заданного в таблице) переводит в бесконечный цикл
Default_Handler:
Infinite_Loop:
	b Infinite_Loop
	.size Default_Handler, .-Default_Handler
 	...
 	.section .vectors
	.word _estack                   @указатель на стэк (конец RAM)
	.word Reset_Handler             @указатель на прерывание RESET
	.word NMI_Handler               @ пока хз че это...  таблица прерываний должна соответствовать datasheet-у
...
*******************************************************************************/
	...
	@.weak	SystemInit  /* переопределение ф-и (ф-я пока не используется)*/
