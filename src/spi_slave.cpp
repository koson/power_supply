...
volatile void SpiSlave::dmaOn() {
    DMA1_Channel4->CPAR = (volatile uint32_t)(&SPI2->DR);  // peripheral address register
    DMA1_Channel4->CMAR = (volatile uint32_t)(receiveBuf); // memory address register
    DMA1_Channel5->CPAR = (volatile uint32_t)(&SPI2->DR);  // peripheral address register
    DMA1_Channel5->CMAR = (volatile uint32_t)(transmitBuf); // memory address register
    DMA1_Channel4->CNDTR = 4; // 4 bytes //renew transfer size
...
volatile void SpiSlave::init() {
    ...
    GPIOB->CRH &= ~GPIO_CRH_CNF15_1; // 1:0 - input with pull-up, pull-down
...
volatile void SpiSlave::DMA_init() {
    for(volatile int i = 0; i<10000; i++);
    RCC->AHBENR |= RCC_AHBENR_DMA1EN;
    for(volatile int i = 0; i<10000; i++);
    //! DMA1 Channel_4 for receiving SPI/I2S2_RX     
    DMA1_Channel4->CPAR = (volatile uint32_t)(&SPI2->DR);  // peripheral address register
    DMA1_Channel4->CMAR = (volatile uint32_t)(receiveBuf); // memory address register
    ...
    //! DMA1 Channel_5 for transmission SPI/I2S2_TX
    DMA1_Channel5->CPAR = (volatile uint32_t)(&SPI2->DR);  // peripheral address register
    DMA1_Channel5->CMAR = (volatile uint32_t)(transmitBuf); // memory address register
    DMA1_Channel5->CNDTR = 4; // 4 bytes
...
extern "C" void SPI2_IRQHandler(void) {
    ...
    if(SPI2->SR & SPI_SR_OVR) {
        SpiSlave::pThis->receivedByte = (volatile uint8_t)SPI2->DR;
